...
class Revision(models.Model):
    ...
    def __str__(self):
        return 'Revision %s of <%s>' % (
            self.id, self.date_created.strftime('%Y-%m-%d %H:%M:%S.%f'))
...
class Change(models.Model):
    ...
    ACTIONS = ((ADDED, _("Added")), (CHANGED, _("Changed")), (DELETED,
                                                              _("Deleted")))
    ...
    user = models.ForeignKey(LOGGING_USER_MODEL,
                             blank=True,
                             null=True,
    ...
    db = models.CharField(
        max_length=191,
        help_text=_(
            "The database the model under version control is stored in."))
    ...
    def __str__(self):
        return "Changes %s of %s <%s>" % (
            self.id, self.object_repr,
            self.date_created.strftime('%Y-%m-%d %H:%M:%S.%f'))
    ...
    def get_changes_by_obj(model, obj_id, related_objects='__all__'):
        ...
        for rel_model in related_objects:
            if isinstance(rel_model, models.OneToOneRel):
                ...
                values = getattr(
                    obj,
                    rel_model.get_accessor_name()).all().values_list('pk',
                                                                     flat=True)
                ...
                values = getattr(
                    obj, rel_model.get_attname()).all().values_list('pk',
                                                                    flat=True)
            else:
                continue
            ...
            history_objects.append({
                'content_type_id':
                ContentType.objects.get_for_model(rel_model.related_model).id,
                'values':
                values
            })
        qobj = models.Q()
        for v in history_objects:
            qobj.add(
                models.Q(content_type_id=v['content_type_id'],
                         object_id__in=v['values']), models.Q.OR)
        return Change.objects.filter(qobj).select_related('user')

    def revert(self):
        with transaction.atomic():
            data = {
                field: values.get('old')
                for field, values in self.display_changed_data().items()
            }
